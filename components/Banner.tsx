      ...
      {message && (
        <div className="fixed top-0 left-0 right-0 flex justify-center z-50 p-4 pointer-events-none">
          <motion.div
            ...
            animate="visible"
            exit="exit"
            className={`pointer-events-auto w-auto max-w-[90%] pl-4 pr-2 py-2 rounded-full shadow-lg border ${bgColor}`}
            role="alert"
          >
            <div className="flex items-center">
              {icon}
              <p className={`font-sans font-semibold text-sm ${textColor}`}>{message}</p>
              <button
                onClick={onClose}
                className={`ml-3 w-7 h-7 flex items-center justify-center rounded-full text-xl leading-none hover:bg-black/10 transition-colors ${textColor} relative top-px`}
